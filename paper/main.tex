...
khususnya pada kawasan hutan lindung. Penelitian ini mengembangkan pipeline
segmentasi spasial indikasi deforestasi menggunakan arsitektur U-Net pada citra
satelit Sentinel-2 dengan 5 kanal masukan (RGB + NIR + NDVI), di mana label
pelatihan dihasilkan secara otomatis melalui temporal NDVI differencing tanpa
anotasi manual. Model dilatih pada 11.500 sampel dari 4 scene, divalidasi pada
2.908 sampel, dan diuji pada 5.695 sampel yang berasal dari 7 scene di Indonesia.
Evaluasi terhadap pseudo-label menunjukkan IoU 0,7256, Dice 0,8410, Precision
0,8340, dan Recall 0,8480 -- peningkatan F1 sebesar 31 poin dari threshold NDVI
langsung (0,53 menjadi 0,84), mengonfirmasi bahwa U-Net mempelajari konteks
spasial di luar perubahan spektral piksel-independen. Makalah ini juga merumuskan
kerangka ambang batas (\textit{threshold}) peringatan dini deforestasi berdasarkan
definisi FAO, regulasi nasional Indonesia, serta temuan ekologi terbaru mengenai
dampak fragmentasi hutan skala kecil terhadap hilangnya karbon dan
...
Perlu dicatat bahwa metrik evaluasi pada Gambar~\ref{fig:test_metrics} mengukur
kesesuaian antara prediksi U-Net dan pseudo-label NDVI, bukan terhadap ground
truth lapangan. Dengan demikian, IoU 0,7256 merepresentasikan kemampuan model
dalam mereplikasi dan menghaluskan keputusan thresholding NDVI dengan konteks
spasial — bukan akurasi deteksi deforestasi absolut. Meskipun demikian,
peningkatan signifikan F1 dari 0,53 (threshold NDVI piksel-independen) menjadi
0,84 (U-Net) pada set uji yang sama menunjukkan bahwa model mempelajari pola
spasial yang tidak dapat ditangkap oleh thresholding piksel independen.
Validasi terhadap ground truth independen (citra resolusi sangat tinggi atau
data lapangan) merupakan langkah berikutnya yang direncanakan untuk memvalidasi
apakah pola spasial yang dipelajari U-Net berkorelasi dengan deforestasi nyata.
...
Penelitian ini berhasil mengembangkan pipeline segmentasi spasial indikasi
deforestasi berbasis U-Net pada citra Sentinel-2 dengan IoU 0,7256 dan Dice
0,8410 pada test set — peningkatan F1 sebesar 31 poin dari threshold NDVI
langsung (0,53 menjadi 0,84). Evaluasi ini dilakukan terhadap pseudo-label
NDVI, bukan ground truth lapangan, sehingga IoU yang dilaporkan merepresentasikan
kemampuan model dalam menghaluskan segmentasi spasial dari sinyal NDVI. Model
mampu melakukan segmentasi pada skala 0,64 hektar — jauh di bawah ambang nasional
Indonesia (6,25 ha) dan ambang ekologis signifikan (2 ha) — sehingga memungkinkan
