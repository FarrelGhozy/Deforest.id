...
Initial results demonstrate 0.73~IoU and 0.84~Dice against
pseudo-labels on hold-out test scenes --- a 31-point F1 improvement
over direct NDVI thresholding (0.53~$\to$~0.84), indicating that
the U-Net learns spatial context beyond pixel-wise spectral change.
The pipeline produces spatially coherent segmentation of
NDVI-derived deforestation indicators at 0.64~ha per chip,
providing a 900-fold improvement over Indonesia's current 6.25~ha
...
\end{itemize}
The system produces spatially coherent segmentation at 64~pixels
(0.64~ha), operating at our minimum alert threshold.

It is important to note that these metrics measure agreement
between U-Net predictions and NDVI-derived pseudo-labels, not
against field ground truth. The IoU of 0.73 therefore represents
the model's ability to replicate and smooth the NDVI thresholding
decision with spatial context --- not absolute deforestation
detection accuracy. Nevertheless, the significant F1 improvement
from 0.53 (direct pixel-wise NDVI threshold) to 0.84 (U-Net) on
the identical test set demonstrates that the model learns spatial
patterns inaccessible to independent pixel thresholding.
Validation against independent ground truth (very-high-resolution
imagery or field data) is planned to confirm whether these spatial
patterns correlate with actual deforestation rather than mere NDVI
replication.
